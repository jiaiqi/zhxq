				...
				<view class="user-head">
					...
					<view class="info-box-top">
						<view class="head-img-wrap">
							<view class="head-img"><text class="cuIcon-myfill text-white"></text></view>
						</view>
					...
					<view class="info-content" v-if="isLogin">
						<view class="area">
							负责区域：
							<view class="role-item active" @click="changeArea(index)" v-for="(role, index) in roleList" :key="role" :class="{ active: curRole === index }">
								<text v-if="role === 'village_user' && roleInfo.villageInfo.village_name">{{ roleInfo.villageInfo.village_name }}</text>
								<text v-if="role === 'street_user' && roleInfo.streetRoadInfo.road_name">{{ roleInfo.streetRoadInfo.road_name }}</text>
								<text v-if="role === 'street_manager' && roleInfo.streetInfo._street_no_disp">{{ roleInfo.streetInfo._street_no_disp }}</text>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="cu-dialog" @click.stop="">
				<view class="change-password bg-white padding-bottom">
					<view class="cu-form-group">
						...
						<input placeholder="请输入旧密码" name="input" class="input" v-model="passwordModel.pwd" />
					</view>
					<view class="cu-form-group">
						<view class="title">新密码</view>
						<input placeholder="请输入新密码" name="input" class="input" v-model="passwordModel.newpwd" />
					</view>
					<view class="cu-form-group">
						<view class="title">确认新密码</view>
						<input placeholder="请再次输入新密码" name="input" class="input" v-model="passwordModel.newpwd2" />
					</view>
...
export default {
	...
	methods: {
		...
		init() {
			if (this.loginUserInfo.login_user_type === 'user') {
				item['data'].push({
					display: '显示',
					icon: 'unbind',
					dest_menu_no: '退出登录',
