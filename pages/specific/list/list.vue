<template>
	<view class="list-wrap" :class="{ 'has-bar': roleInfo.streetRoadInfo }">
		<view class="cu-bar fixed bg-white" v-if="roleInfo.streetRoadInfo">
			<view class="action"><button class="cu-btn bg-blue shadow-blur round add-btn" @click="toAdd">+</button></view>
		</view>
			...
			<view class="list-item" v-for="item in list" :key="item.id" @click="toDetail(item)">
				<view class="item-top">
					...
					<view class="item-title">
						...
						<view class="value">{{ item.check_date || '' }}</view>
					</view>
				</view>
				<view class="item-content">
					<view class="result-remark">
						<view class="title">问题说明：</view>
						<view class="text-area result-content">{{ item.remark || '' }}</view>
					</view>
					<view class="result-remark" @click.stop="" v-if="item.file_no">
						<view class="title">巡查附件：</view>
						<view class="result-image result-content" @tap="toPreviewImage(item.imageList.map(e => e.originUrl))">
							<image class="image" :src="pic.originUrl" v-for="pic in item.imageList"></image>
						</view>
					</view>

					<view class="result-remark" v-if="item.report_txt">
						<view class="title">整改说明</view>
						<view class="text-area result-content">{{ item.report_txt || '' }}</view>
					</view>
				</view>
				<view class="item-footer" v-if="item.proc_status === '巡查问题发布'">
					<view class="cu-bar bg-white">
						...
						<view class="action"><button class="cu-btn bg-blue shadow-blur " @click.stop="toDetail(item)">详情</button></view>
					</view>
				</view>
				<!-- 街长已检查 第三步 -->
...
export default {
	...
	methods: {
		...
		toAdd() {
			...
			// })
			uni.navigateTo({
				url: '/pages/specific/add/checkRecord/checkRecord'
			});
		},
		toDetail(e) {
			if (e.record_no) {
