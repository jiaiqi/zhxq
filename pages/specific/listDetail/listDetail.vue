			...
			<view class="list-item">
				<view class="first-step" :class="{'second-step':detail.proc_status}">
					...
					<view class="item-content" v-if="detail.check_result!=='全部通过'">
						...
						<view class="result-remark">
							...
							<view class="result-image result-content" v-if="detail.file_no"
								@click="toPreviewImage(detail.imageList.map(e=>e.originUrl))">
				...
				<view class="second-step" :class="{'third-step':detail.proc_status==='巡查结果确认'}">
					<view class="cu-form-group margin-top border-top">
						...
						<robby-image-upload :enableAdd="!detail.report_status" :enableDel='!detail.report_status'
							v-model="imgList" :enable-del="true" :server-url="upLoadUrl" :form-data="formDatas"
					...
					<view class="cu-form-group">
						<textarea maxlength="-1" v-model="fieldModel.report_txt" @input="textareaAInput"
							placeholder="整改说明" class="text-area"></textarea>
					</view>
					<view class="item-footer">
						<view class="title cu-bar">
							<text>整改结果</text>
							<view class="result" v-if="detail.report_status">
								{{detail.report_status}}
							</view>
						</view>
						<radio-group class="block" @change="RadioChange" v-if="!detail.report_status">
							<radio class="margin-right" :class="radioVal==radio.value?'checked':''"
								:checked="radioVal==radio.value?true:false" v-model="radio.value"
								v-for="radio in radioList">{{radio.label}}</radio>
						</radio-group>
					</view>
					<view class="center" v-if="detail.proc_status==='巡查问题发布'">
						<button type="primary" class="cu-btn bg-blue margin-top"
							@click="updateDetail(false)">整改确认</button>
					</view>
				</view>
				<view class="third-step" v-if="detail.proc_status!=='巡查问题发布'">
					<view class="item-footer margin-top"
						v-if="detail.proc_status==='完成'||(detail.proc_status==='巡查结果确认'&&roleInfo.villageInfo)">
						<view class="title"><text class="text-red margin-right-xs"></text>整改结果评分</view>
						<input type="digit" class="text-area" :disabled="!isVillageManage"
							v-model.number="confirmModel.score" placeholder="输入整改结果评分" />
					</view>
					<view class="item-footer margin-top"
						v-if="detail.proc_status==='完成'||(detail.proc_status==='巡查结果确认'&&roleInfo.villageInfo)">
						<view class="title"><text class="text-red margin-right-xs"></text>整改结果说明</view>
						<textarea class="text-area high-height" :disabled="!isVillageManage"
							v-model="confirmModel.confirm_txt" placeholder="输入整改结果说明" />
					</view>
					<view class="center" v-if="detail.proc_status==='巡查结果确认'&&isVillageManage">
						<button type="primary" class="cu-btn bg-blue margin-top"
							@click="updateDetail(true)">整改结果确认</button>
					</view>
				</view>
			</view>
		</view>
			...
			async selectDetail() {
				...
				if (res.data.state === 'SUCCESS' && Array.isArray(res.data.data) && res.data.data.length > 0) {
					...
					if (detail.report_file_no) {
						...
						if (fileDatas) {
							for (let i = 0; i < fileDatas.length; i++) {
								this.imgList.push(this.$api.getFilePath + fileDatas[i].fileurl + '&bx_auth_ticket=' +
									uni
									.getStorageSync('bx_auth_ticket'));
							}
						}
