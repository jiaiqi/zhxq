<template>
	<view style="min-height: 100vh;background-color: #fff;">
		...
		<view class="road-info bg-blue" v-if="loginUserInfo.real_name || loginUserInfo.user_disp">
			负责人：{{ loginUserInfo.real_name || loginUserInfo.user_disp }}</view>
		<view class="add-form">
			...
			<view class="cu-form-group">
				...
				<bx-radio-group class="input" v-model="formData.general_result" @change="changeStatus">
					<bx-radio class="radio" color="#2979ff" v-for="item in options2" :key="item.value"
						:name="item.value">{{ item.label }}</bx-radio>
				</bx-radio-group>
			</view>
			<view class="" v-if="formData.general_result == '有问题'">
				...
				<view class="cu-form-group">
					<view class="title"><text class="text-red margin-right-xs">*</text>存在问题照片</view>
					<robby-image-upload style="flex: 1;" v-model="imgList" :enable-del="true" :server-url="upLoadUrl"
						:form-data="formDatas" :header="reqHeader" :server-url-delete-image="deleteFileUrl"
						@add="getImageInfo" @delete="deleteImage" :showUploadProgress="true" :limit="9">
					</robby-image-upload>
				</view>
				<view class="cu-form-group">
					<view class="title">
					...
					<textarea maxlength="-1" placeholder="整改问题说明" class="text-area"
			...
			<view class="button-box"><button type="primary" class="button cu-btn bg-blue"
					@click="submitForm">提交</button></view>
		</view>
	</view>
</template>
		...
		methods: {
			...
			changeStatus(e) {
				Object.keys(this.formData).forEach(key => {
					...
					if (e === '有问题') {
						if (['col_1', 'col_2', 'col_3', 'col_4', 'col_5', 'col_6', 'col_7', 'col_8', 'col_9']
							.includes(key)) {
							this.formData[key] = '√';
						}
					} else if (e === '没问题') {
			...
			async submitForm(e) {
				...
				if(this.formData.general_result==='有问题'){
