<template>
	<view>
		<view class="road-info bg-blue">{{ roadInfo || '' }}</view>
		<view class="road-info bg-blue" v-if="loginUserInfo.real_name || loginUserInfo.user_disp">负责人：{{ loginUserInfo.real_name || loginUserInfo.user_disp }}</view>
		<view class="add-form">
			...
			<view class="cu-form-group">
				...
				<picker mode="date" v-model="formData.check_date" :disabled="true" @change="DateChange">
					<view class="uni-input">{{ formData.check_date || '请选择' }}</view>
				</picker>
			</view>
			<view class="cu-form-group">
				<view class="title">
				...
				<bx-radio-group class="input" v-model="formData.general_result" @change="changeStatus">
					<bx-radio class="radio" color="#2979ff" v-for="item in options2" :key="item.value" :name="item.value">{{ item.label }}</bx-radio>
				</bx-radio-group>
			</view>
			<view class="" v-if="formData.general_result == '有问题'">
				<view class="cu-form-group">
					<view class="title">
						<text class="text-red margin-right-xs"></text>
						垃圾乱倒
					...
					<bx-radio-group class="input" v-model="formData.col_1">
						<bx-radio class="radio" color="#2979ff" v-for="item in options" :key="item.value" :name="item.value">{{ item.label }}</bx-radio>
					</bx-radio-group>
				</view>
				<view class="cu-form-group">
					<view class="title">
			...
			<view class="cu-form-group">
				<view class="title w100">存在问题照片</view>
				<robby-image-upload
					v-model="imgList"
					:enable-del="true"
			...
			<view class="cu-form-group"><textarea maxlength="-1" placeholder="整改问题说明" class="text-area" v-model="formData.remark"></textarea></view>
			<view class="cu-form-group">
				<view class="title">
					<text class="text-red margin-right-xs"></text>
					整改时限
			...
			<view class="cu-form-group" v-if="formData.report_over_time">
				...
				<picker mode="date" :disabled="true" v-model="formData.report_over_time" start="2021-04-01" end="2022-09-01" @change="DateChange2">
					<view class="uni-input">{{ formData.report_over_time }}</view>
				</picker>
			</view>
			<view class="button-box"><button type="primary" class="button cu-btn bg-blue" @click="submitForm">提交</button></view>
		</view>
	</view>
</template>
	...
	methods: {
		...
		overTimeChange(info) {
			...
			if (e === '当日整改完毕') {
				...
				this.formData.report_over_time = dayjs()
					.add(2, 'day')
		...
		changeStatus(e) {
			Object.keys(this.formData).forEach(key => {
				...
				if (e === '有问题') {
					if (['col_1', 'col_2', 'col_3', 'col_4', 'col_5', 'col_6', 'col_7', 'col_8', 'col_9'].includes(key)) {
						this.formData[key] = '√';
					}
				} else if (e === '没问题') {
					if (['col_1', 'col_2', 'col_3', 'col_4', 'col_5', 'col_6', 'col_7', 'col_8', 'col_9'].includes(key)) {
