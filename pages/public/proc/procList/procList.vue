<template>
	<view>
		<bx-list v-if="listConfig.serviceName && listConfig.listConfig" ref="bxList" :srvApp="app" showLabel
			:pageType="listConfig.listType" :rowButtons="listConfig.rowButtons" :serviceName="listConfig.serviceName"
			:condition="listConfig.condition" :listType="listConfig.listType" :viewTemp="listConfig.viewTemp"
			:isWy="isWy" :listConfig="listConfig.listConfig" :searchWords="listConfig.searchVal"
			:queryLeftWord="listConfig.queryLeftWord" :searchColumn="'name'" :detailList="listConfig.detailList"
			:rownumber="5" :fixed="true" :top="listTop" :heightStyle="'calc(100vh-220upx)'" :order="listConfig.order"
		...
		<view class="cu-modal bottom-modal" :class="showModal ? 'show' : ''">
			<view class="cu-dialog" style="max-height: 800upx;">
				...
				<view class="padding-xl" v-if="listConfig.listConfig">
					<bxform ref="bxformApplyProc" :pageType="'add'" :fields="listConfig.listConfig.srv_cols"
						:BxformType="'add'" @value-blur="valueChange"></bxform>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			toApply() {
				...
				uni.navigateTo({
					url: '../apply/apply?serviceName=' + this.listConfig.addService + '&cond=' + JSON.stringify(
						cond)
				});
			},
			async applyFlow() {
			...
			clickFootBtn(e) {
				...
				if (e.button.button_type === 'edit') {
					...
					console.log('======', e);
					uni.navigateTo({
						url: '/pages/specific/permit/permit?code=' + e.row.qr_code + '&rowData=' +
							encodeURIComponent(JSON.stringify(e.row))
					});
					// this.procNo = e.row.proc_instance_no
					...
					if (e.button.servcie_type === 'delete') {
						if (e.button.operate_params && e.button.operate_service) {
							...
							let url = this.getServiceUrl(uni.getStorageSync('activeApp'), e.button.operate_service,
								'operate');
							let req = [{
								serviceName: e.button.operate_service,
								condition: condition,
								data: [{}],
								srvApp: uni.getStorageSync('activeApp')
							}];
							this.$http
								.post(url, req)
								.then(res => {
						...
						uni.navigateTo({
							url: '/pages/public/formPage/formPage?params=' + decodeURIComponent(JSON.stringify(
								params))
						});
					}
				} else if (e.button.button_type === 'applyProc' && e.application === 'zhxq' && e.service_name ===
					'srvzhxq_syrk_select') {}
			},
			...
			async getListV2() {
				...
				if (serviceName === 'srvdaq_street_inspect_record_select') {
					colVs.rowButton = [{
						page_type: '流程列表',
						button_name: '流程详情',
						button_type: 'procdetail',
						service_name: 'srvdaq_street_inspect_record_select',
						service_view_name: '街道办巡查查询',
						permission: true,
						application: 'daq',
						page_area: '表格行按钮'
					}]
		...
		onShareAppMessage(res) {
			...
			// };
			if (res.target.dataset.info && res.target.dataset.info.button_type === 'applyProc' && res.target.dataset.info
				.button_name === '邀请' && res.target.dataset.procno) {
				return {
					title: '绑定住户邀请',
