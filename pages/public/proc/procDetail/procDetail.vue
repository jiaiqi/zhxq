	...
	<view class="proc-wrap">
		...
		<scroll-view scroll-x class="bg-white nav">
			<view class="flex text-center">
				<view class="cu-item flex-sub" :class="index == TabCur ? 'current-tab cur' : ''"
		...
		<view class="detail-view content-box" v-if="TabCur === 0">
			<view class="detail-form">
				<bxform ref="bxDetailForm" :pageType="type" :BxformType="'detail'" :fields="fields"></bxform>
			</view>
		</view>
        ...
        ">
				<text class="cu-btn bg-blue margin-right" v-if="
		...
		<uni-popup ref="approvalPopup" type="bottom">
			<view class="form-view" v-if="stepsCfgData.length > 0">
				<view class="" style="width: 100%;" v-for="(item, index) in stepsCfgData" :key="index">
					<bxform :procData="activityData" :ref="'bxFormStep' + index" :pageType="item.formType"
						:BxformType="item.formType" :fields="item.fields"></bxform>
				</view>
				<bxform :procData="activityData" v-if="isHandler && activityData" ref="approvalForm"
					class="approvalForm" :pageType="'detail'" :BxformType="'detail'" :fields="approvalFormCfg"></bxform>
				<bxform v-if="
            procBasicConfig.proHanleData &&
            procBasicConfig.proHanleData.activeStep !== 0 &&
            !isHandler &&
            activityData &&
            detailFiledData
          " ref="approvalForm" :pageType="'add'" class="approvalForm" :BxformType="'add'" :procData="activityData"
					:fields="approvalFormCfg" :detailFiledData="detailFiledData"></bxform>
		...
		<uni-popup ref="recordPopup" type="bottom">
			<view class="form-view" v-if="currentRecord.fields && currentRecord.fields.length > 0">
				<bxform ref="bxFormRecord " pageType="detail" BxformType="detail" :fields="currentRecord.fields">
				</bxform>
	...
	export default {
		...
		methods: {
			...
			async getProcRecord() {
				...
				let res = await this.onRequest('select', 'srvprocess_record_select', req, this.srvInfo.app ? this
					.srvInfo.app : 'zhxq');
				if (res.data.state === 'SUCCESS') {
				...
				if (this.procBasicConfig.proHanleData.activeStep === 0) {
					...
					for (let i = 0; i < this.stepsCfgData.length; i++) {
						...
						if (item.formType) {
							...
							let res = await this.onRequest('apply', serviceName, req, this.srvInfo.app ? this.srvInfo
								.app : 'zhxq');
							if (res.data.state === 'SUCCESS') {
					...
					let res = await self.onRequest('process', 'approval', reqData, this.srvInfo.app ? this.srvInfo
						.app : 'zhxq');
					// .then(res => {
					if (res.data.state === 'SUCCESS') {
			...
			async getColV2(serviceName, type) {
				let self = this;
				let colVs = await this.getServiceV2(serviceName, type, type, this.srvInfo.app ? this.srvInfo.app :
					'zhxq');
