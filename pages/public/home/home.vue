		...
		async clickMenu(e) {
			...
			if (e.type && e.type === 'more') {
				...
				if (e.dest_app) {
					...
					if (e.dest_page && e.dest_page.indexOf('/pages/public/list/list') !== -1 && viewTemp) {
					...
					this.getPageItem(item).then(data => {
						if (item.div_type === 'buttons') {
							data = data.filter(dataItem => {
								...
								if (['街道办巡查', '街道考核'].includes(dataItem.dest_menu_no) && !roleInfo.streetInfo && !roleInfo.villageInfo) {
									// 村长和街道办能看
									return false;
								}
								if (['街长考核'].includes(dataItem.dest_menu_no) && !roleInfo.villageInfo) {
									// 村长和街长能看
									return false;
								}
						...
						if (item.div_type === 'buttons') {
							...
							if (this.loginUserInfo && this.loginUserInfo.login_user_type === 'user') {
								item['data'].push({
									display: '显示',
