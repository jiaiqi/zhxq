<template>
	<view>
		<bxform :service="serviceName" ref="bxForm" :pageType="type" :BxformType="type" :fields="fields"
			:moreConfig="colsV2Data && colsV2Data.more_config ? colsV2Data.more_config : null"></bxform>
							...
							this.defaultCondition.forEach(cond => {
								colVs._fieldInfo.forEach(field => {
									if (cond.colName === field.column && field.isRequire !== true) {
										...
										field['display'] = false;
										field['showExp'] = false;
									} else if (cond.colName === field.column && field.isRequire ===
										true) {
										field['value'] = cond['value'];
									}
								});
				...
				switch (e.button_type) {
					...
					case 'submit':
						if (req) {
							...
							let id = req.picp;
							let openid = uni.getStorageSync('login_user_info').openid;
							...
							if (false) {
								...
								// 						let app = uni.getStorageSync('activeApp');
								// 						let url = self.getServiceUrl(app, e
								// 							.service_name, 'add');
								// 						if (hasBasicInfo !== false) {
								// 							// update
								// 							url = self.getServiceUrl(app, e
								// 								.service_name.replace('_add',
								// 									'_update'), 'operate');
								// 							req = [{
								// 								serviceName: e.service_name
								// 									.replace('_add',
								...
								// 							uni.setStorageSync('basics_info',
								// 								res.data?.response[0]
								// 								?.response?.effect_data[0]);
								// 							if (res.data.state === 'SUCCESS') {
								// 								uni.showModal({
								// 									title: '提示',
								// 									content: hasBasicInfo ===
								// 										false ?
								// 										'登记成功' :
								// 										'关联成功',
								// 									showCancel: false,
											...
											self.$http.post(url, req).then(res => {
												...
												if (res.data.state === 'SUCCESS') {
													if (res.data && Array.isArray(res.data
															.response) && res.data.response.length >
														0 &&
														res.data.response[0].response && Array.isArray(
															res.data.response[0].response.effect_data
														) && res.data.response[0].response
														.effect_data.length > 0
													) {
														self.$store.commit('SET_STAFF_INFO', res.data
															.response[0].response.effect_data[0])
													}

													// console.log('res.data', res.data.response[0]
													// 	.response.effect_data[0]);
													// uni.setStorageSync('basics_info', res.data
													// 	?.response[0]?.response?.effect_data[0]);
													uni.showModal({
														title: '提示',
														content: '登记成功,请等待管理人员审核信息',
														showCancel: false,
														success(res) {
															if (res.confirm) {
																// self.wxLogin();
																...
																// if (self.house_no) {
																// 	uni.redirectTo({
																// 		url: `/pages/public/formPage/formPage?q=https%3A%2F%2Fwx2.100xsys.cn%2Fxuncha%2F${self.house_no}`
																// 	})
																// } else {
																// 	uni.navigateBack({
																// 		delta: 1
