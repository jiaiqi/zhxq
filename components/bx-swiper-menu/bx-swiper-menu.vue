<template>
	<view class="menu-list">
		...
		<swiper class="round-dot" :indicator-dots="false" :circular="true" :autoplay="false" interval="5000"
			duration="500" style="height: auto;max-height: 380upx;overflow: hidden;"
			v-if="menus&&Array.isArray(menus)&&menus.length > 10">
			<swiper-item>
				<view class="bg-white  menu-item" v-if="menus&&Array.isArray(menus)&&menus.length <= 10">
					<view :class="{ 'bg-imgs': !item.icon, 'menu-image': item.icon }" v-for="(item, index) in menus"
						:key="index" :style="{ width: menus.length === 5 || menus.length > 8 ? '20%' : '25%' }">
						<view class="menu-pic bg-blue" @click="clickMenu(item)" v-if="!item.icon">
							{{ item[menuTemplate['lableCol']].slice(0, 2) }}
						</view>
						<view class="menu-pic" @click="clickMenu(item)" v-if="item.icon">
...
<script>
	export default {
		...
		computed: {
			menuColSpan() {
				if (this.config && this.config.grid_hight_span) {
					return this.config.grid_hight_span
				} else if (Array.isArray(this.menuList) && this.menuList.length > 8) {
					return 5
				} else {
					return 4
				}
