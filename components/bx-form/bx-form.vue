	...
	<view class="bg-white bx-steps" style="width:100%;" v-if="allField.length > 0">
		<view v-for="(item, index) in allField" :key="index">
			<formItem :procData="procData" :field="item" :pageFormType="BxformType" :showTextarea="showTextarea"
				...
				@seatPostion="getSeatInfo" @on-form-item="onItemButtons($event)" @on-value-change="onValChange($event)"
		...
		methods: {
			...
			onValChange(e) {
				...
				if (e && e.column === "check_result") {
					if (this.service === 'srvdaq_street_check_record_add') {
						if (e.value === '需整改') {
							this.allField.forEach((item, index) => {
				...
				if (this.service === 'srvdaq_street_personnel_exam_add' || this.service ===
						...
						if (colArr.every(item => item.value)) {
							if (serviceCallCfg.req.condition) {
								...
								serviceCallCfg.req.condition.forEach(cond => {
									if (cond.value.valueType && cond.value.valueType === 'rowData' && cond
										.value.valueKey) {
										condition.push({
