<template>
	<view class="bx-form-item">
		<view class="cu-form-group itemwrap" v-if="fieldData.display"
			...
			<view class="title" :class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }">
				<text> <text class="text-red text-shadow" v-show="fieldData.isRequire">*</text>
				...
				<view class="help-tip" v-if="fieldData.moreConfig&&fieldData.moreConfig.help_tips">
					{{fieldData.moreConfig.help_tips}}
				</view>
			</view>
			<!-- 图片描述 -->
			...
			<view v-if="pageFormType === 'detail'" class="detail-text">
				...
				<rich-text :nodes="fieldData.value"
			...
			<view class="form-content" :class="{
				...
				<radio-group @change="radioChange" v-if="fieldData.type === 'radio'"
					:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }">
					<radio color="#0bc99d" :key="index" :disabled="fieldData.disabled ? fieldData.disabled : false"
						v-for="(itema, index) in fieldData.options" class="radio" :checked="itema === fieldData.value"
						:value="itema" style="transform: scale(0.7);">
						<span style="flex: 1;padding-left: 10rpx;transform: scale(1.2)">{{ itema }}</span>
					</radio>
				</radio-group>

				<bx-radio-group v-model="fieldData.value" @change="radioChange" v-else-if="fieldData.type === 'radioFk'"
					:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }">
					<bx-radio class="radio" color="#2979ff" v-for="item in fieldData.options"
						:disabled="fieldData.disabled ? fieldData.disabled : false" :key="item.id" :name="item.value"
				...
				<checkbox-group name="checkbox-group" class="checkbox-group" v-else-if="fieldData.type === 'checkbox'"
					...
					<label v-for="(item, index) in fieldData.options" :key="index" class="checkbox"
						...
						<checkbox color="#2979ff" :value="item"
				...
				<checkbox-group name="checkbox-group" class="checkbox-group" v-else-if="fieldData.type === 'checkboxFk'"
					...
					<label v-for="(item, index) in fieldData.options" :key="index" class="checkbox"
						...
						<checkbox color="#2979ff" class="blue"
							:disabled="fieldData.disabled ? fieldData.disabled : false"
				...
				<view v-else-if="fieldData.type === 'images'">
					<robby-image-upload :value="imagesUrl" :enable-del="fieldData.disabled ? !fieldData.disabled : true"
						:enable-add="fieldData.disabled ? !fieldData.disabled : true" :server-url="upLoadUrl"
						:form-data="formData" :header="reqHeader" :server-url-delete-image="deleteFileUrl"
						@delete="deleteImage" @add="getImageInfo" :showUploadProgress="true" :limit="fieldData.fileNum">
					</robby-image-upload>
				</view>
				...
				<textarea style="min-height: 60px;width: 100%;"
					...
					@blur="onInputBlur" auto-height v-model="fieldData.value" @input="onInputChange"
					:disabled="fieldData.disabled ? fieldData.disabled : false"
					:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }"
					v-else-if="fieldData.type === 'textarea' && showTextarea"
					:placeholder="'输入' + placeholderValue"></textarea>
				<view @click="showRichText = true"
					v-if="fieldData.type === 'richText' && !fieldData.disabled && !fieldData.value">
					{{ '点击编辑' + placeholderValue }}
				</view>
				<rich-text class="rich-text-content" @click="showRichText = true"
					:placeholder="'点击编辑' + placeholderValue" :nodes="fieldData.value"
					v-if="!fieldData.disabled && (fieldData.type === 'snote' || fieldData.type === 'Note' || fieldData.type === 'richText')">
				</rich-text>
				<picker class="pickers" @change="PickerChange($event, fieldData)" :disabled="fieldData.disabled"
					:value="index" :range="picker" v-if="fieldData.type === 'poupchange'">
					<input type="text" :placeholder="'点击选择' + placeholderValue" :value="picker[index]"
						:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }" name="input"
						:disabled="true" />
				</picker>
				<!--  <bx-editor
          :field="fieldData"
        ...
        ></bx-editor> -->
				<view class="content padding-0"
					...
					<input @blur="onInputBlur" @input="onInputChange" v-model="fieldData.value" disabled
						class="date-input" @click.stop="fieldData.disabled ? false : toggleTab(fieldData.type)"
						:placeholder="'请选择' + placeholderValue"
						:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }" style="width: 100%;"
						name="input" />
					<text class="input-icon cuIcon-calendar"
						style="position: absolute;top:10upx;right: 20upx;color: #0bc99d;"
						@click.stop="fieldData.disabled ? false : toggleTab(fieldData.type)"></text>
					...
					<w-picker mode="time" :current="true" :second="false" @confirm="onConfirm" ref="Time" step="1">
					</w-picker>
				</view>
				...
				<input @blur="onInputBlur" type="number" :placeholder="'输入' + placeholderValue" @input="onInputChange"
				...
				<input @blur="onInputBlur" type="digit" :placeholder="'输入' + placeholderValue" @input="onInputChange"
					:disabled="fieldData.disabled ? fieldData.disabled : false"
					:max="fieldData.item_type_attr && fieldData.item_type_attr.max ? fieldData.item_type_attr.max : 999"
					:min="fieldData.item_type_attr && fieldData.item_type_attr.min ? fieldData.item_type_attr.min : 0"
					v-model.number="fieldData.value"
				...
				<view class="input-view" v-else-if="fieldData.type==='treeSelectorPro'">
					<input :placeholder="fieldData.placeholder ? fieldData.placeholder : '点击选择' + placeholderValue"
						:value="treeSelectorShowValue ? treeSelectorShowValue : fieldData.value" disabled
						:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }" name="input" class="input"
						@click="openTreeSelectorPro" />
				</view>
				...
				<view v-else-if="fieldData.type === 'cascader'" @click="openCascader">
					<input :placeholder="'点击选择' + placeholderValue" v-model="fieldData.value" disabled
						:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }" name="input" />
				</view>
				...
				<view class="item-group flex align-center" style="" v-else-if="fieldData.type === 'input'">
					<input @blur="onInputBlur"
						:maxlength="fieldData.item_type_attr && fieldData.item_type_attr.max_len ? fieldData.item_type_attr.max_len : 100"
						:disabled="fieldData.disabled ? fieldData.disabled : false"
						:placeholder="'输入' + placeholderValue" v-model="fieldData.value" @input="onInputChange"
						:class="{'valid_error':!valid.valid,'disabled':fieldData.disabled }" name="input" type="text" />
				</view>
				<!-- 字段按钮组 -->
			...
			<view class="cu-dialog">
				<view class="set-box" v-if="isArray(checkboxList)">
					<bx-checkbox-group checkboxMode="button" v-model="checkedValue">
						<bx-checkbox v-model="item.checked" v-for="(item, index) in checkboxList" :key="item.label"
							:name="item.value">{{ item.label }}</bx-checkbox>
					</bx-checkbox-group>
				...
				<view class="tree-selector">
					<view class="cu-bar search bg-white">
						<view class="search-form round">
							...
							<input @input="getTreeSelectorDataWithKey" @confirm="getTreeSelectorDataWithKey"
								v-model="treeSearchVal" :adjust-position="false" type="text" confirm-type="search" />
						</view>
					...
					<bxTreeSelector :srvInfo="isArray(fieldData.option_list_v2) ? null : fieldData.option_list_v2"
						...
						@clickParentNode="onTreeGridChange" @clickLastNode="onMenu" class="selector-wrap">
					</bxTreeSelector>
					<view class="no-data" style="letter-spacing: 2px;line-height: 300rpx;color: #007AFF;"
					...
					<view class="dialog-button">
						<view class="cu-btn bg-blue shadow button" @tap="showTreeSelector = false">取消</view>
					</view>
				</view>
			</view>
		</view>
		<view class="cu-modal bottom-modal" :class="{ show: showRichText }" @tap.self="showRichText = false">
			<view class="cu-dialog rich-text">
				<jin-edit :html="richTextVal" @editOk="editOk" ref="richText" :uploadFileUrl="upLoadUrl"
					:form-data="formData" :header="reqHeader" :server-url-delete-image="deleteFileUrl"></jin-edit>
			</view>
		</view>
		<view class="cu-modal bottom-modal" :class="{show:showTreeSelectorPro}" v-if="showTreeSelectorPro">
			<view class="cu-dialog">
				<tree-selector-pro @getCascaderValue="getCascaderValue" :srvInfo="fieldData.srvInfo">
				</tree-selector-pro>
			</view>
		</view>
		<uni-popup ref="popup" type="bottom" @change="changePopup" v-if="fieldData.type === 'cascader'">
...
<script>
	...
	import treeSelectorPro from '@/components/cascader-selector/cascader-selector.vue';
	...
	export default {
		...
		computed: {
			...
			dictShowValue() {
				...
				if (option_list_v2 && Array.isArray(option_list_v2) && option_list_v2.length > 0 && this.fieldData
					.col_type === 'Dict') {
					let val;
		...
		methods: {
			...
			async getpoupInfo(info) {
				...
				if (res.data.state === 'SUCCESS') {
					...
					res.data.data.forEach(item => {
						if (srvInfo && srvInfo.key_disp_col && item[srvInfo.key_disp_col]) {
							resData.push(item[srvInfo.key_disp_col]);
						} else {
							resData.push(item.name);
						}
					});
					if (this.fieldData.value) {
						this.index = res.data.data.findIndex(item => item[srvInfo.refed_col] === this.fieldData.value)
						let oriItem = this.oriPicker[this.index]
			...
			getCascaderValue(val, btnType) {
				if (btnType === 'sure') {
					...
					this.showTreeSelectorPro = false
					if (val) {
						...
						if (this.fieldData.srvInfo.showCol) {
							this.treeSelectorShowValue = val[this.fieldData.srvInfo.showCol];
						}
					} else {
			...
			addSelectorItem() {
				...
				let fieldsCond = [{
					column: config.key_disp_col,
					value: this.treeSearchVal
				}];
				uni.navigateTo({
					url: '/pages/public/formPage/formPage?refreshWithKey=true&serviceName=' + serviceName +
						'&type=add&fieldsCond=' + JSON.stringify(fieldsCond),
					success() {
						// this.treeSearchVal = '';
					}
				});
			...
			async getTreeSelectorData(cond, serv, relation_condition) {
				...
				if (res.data.state === 'SUCCESS' && Array.isArray(res.data.data)) {
					...
					if (hasParentNo) {
						self.treeSelectorData = self.treeReform(res.data.data, 'parent_no', 'no', self.fieldData
							.option_list_v2);
