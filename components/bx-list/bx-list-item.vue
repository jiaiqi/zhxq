<template>
	<view class="list-item-wrap bg-white" :class="{'grid-layout-item':layout==='grid'}">
		<view class="list-item flex" v-if="viewType === 'normal'">
			<imgLazy class="main-image" v-if="itemData[viewTemp['img']]" :item="goodsData" :imgColName="'img'">
			</imgLazy>
			<view class="content-box flex-twice" v-if="listType === 'proc' && pageType === 'proc'">
				<view class="content-header">
					<view class="title" @click="listItemClick" v-if="itemData[viewTemp.title]">
						{{ itemData[viewTemp.title] }}</view>
					<view class="title" @click="listItemClick" v-else>
						<text v-if="itemData.road_name&&itemData.village_name">
							{{itemData.village_name}}-{{itemData.road_name}}/{{itemData._road_manager_disp||''}}
						</text>
						<text v-if="!itemData.road_name&&itemData.village_name">
							{{itemData.village_name}}/{{itemData._village_manager_disp||''}}
						</text>
					</view>
				...
				<view class="footer" v-if="rowButton.length > 0">
					<view class="footer-btn" v-if="showFootBtn">
						<text v-if="deRowButDisplay(itemData, item) && !detailList"
							...
							v-for="item in rowButton" @click="footBtnClick(item)">
							{{ item.button_name }}
						</text>
					</view>
				</view>
